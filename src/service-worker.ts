...
sw.addEventListener('install', (event) => {
	event.waitUntil(
		(async () => {
			...
			// the host may 302 the root to /index.html; a response marked "redirected"
			// is rejected by browsers when replayed for a navigation, so re-wrap the
			// shell body in a clean response before caching it
...
sw.addEventListener('fetch', (event) => {
	...
	event.respondWith(
		(async () => {
			...
			// never cache redirected responses — they can't answer future navigations
			if (response.ok && !response.redirected && new URL(request.url).origin === location.origin) {
